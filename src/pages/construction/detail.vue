<template>
  <view class="page">
    ...
    <!-- 完工提交悬浮按钮 -->
    <view
      v-if="allSubTasksCompleted && task.status === 'EXECUTING'"
      class="submit-fab"
      @click="handleSubmitConstruction"
    >
      <text class="submit-fab-text">提交完工</text>
    </view>
  </view>
</template>
...
const allSubTasksCompleted = computed(() => {
  return (
    ...
    subTaskList.value.every((sub) => sub.status === "COMPLETED")
  );
});

async function handleSubmitConstruction() {
  try {
    await doConstructionSubmit({ taskId: task.value.id });
    uni.showToast({ title: "提交成功", icon: "success" });
    // 刷新详情
    fetchDetail(task.value.id);
  } catch (error) {
