<template>
    <view class="page">
        <!-- 顶部导航栏 -->
        <wd-navbar title="消息中心" left-arrow fixed placeholder @click-left="goBack">
            <template #right>
                <view
                    v-if="unreadTotal > 0"
                    class="read-all-btn"
        ...
        <!-- 顶部全部/未读分类 Tab 切换 -->
        <view class="tabs-box">
            <wd-tabs v-model="activeTab" color="#0066ff" inactive-color="#999" @change="handleTabChange">
        ...
        >
            <view class="content-container">
                <view v-for="msg in messageList" :key="msg.id" class="message-card" @click="handleMessageClick(msg)">
                    <!-- 左侧类型专属图标 -->
                    <view class="icon-wrap">
                        <view v-if="msg.unreadCount > 0" class="unread-dot"></view>
                        <view class="icon-circle" :class="msg.typeClass || 'bg-blue'">
                            <wd-icon :name="msg.iconName || 'notification'" size="24px" color="#fff" />
                        </view>
                    </view>
                ...
                <view v-if="messageList.length > 0" class="load-more-box">
                    <text v-if="loadMoreStatus === 'loading'" class="load-more-text">加载中...</text>
                    <text v-if="loadMoreStatus === 'nomore'" class="load-more-text">没有更多了</text>
                </view>
...
import { getMessagePage, getUnreadCount, markAsRead, markAllAsRead } from '@/api'
